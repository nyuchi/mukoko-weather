...
class TestGenerateSummary:
    ...
    async def test_cache_write_failure_still_returns_insight(self, mock_db, mock_cache,
                                                              ...
                                                              mock_set):
        """Serve-first discipline: the insight is already generated by the
        time the cache write runs, so a blocked write (storage quota,
        outage) must never turn a good response into a 500."""
        mock_db_inst = MagicMock()
        mock_db.return_value = mock_db_inst
        mock_db_inst.__getitem__ = MagicMock(return_value=MagicMock(
            find_one=MagicMock(return_value=None)
        ...
        mock_season.return_value = {"name": "Summer", "localName": "Summer",
                                     "description": "Warm season with possible thunderstorms"}
        mock_set.side_effect = Exception("you are over your space quota")
